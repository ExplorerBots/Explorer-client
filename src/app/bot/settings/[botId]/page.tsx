'use client';
import { BotContext, BotProvider } from '@/app/bot/BotProvider';
import { useAddToWhitelist } from '@/hooks/bot/settings/useAddToWhitelist';
import { useRemoveToWhitelist } from '@/hooks/bot/settings/useRemoveToWhitelist';
import { IBot, IBotWhitelistUser } from '@/interfaces';
...
const BotSettingsPage = ({ params }: { params: any }) => {
   ...
   const handleAddToWhitelist = async (username: string) => {
      ...
      const response: any = await addToWhitelist({
         botId: bot!.id,
         username,
      });
   ...
   const handleRemoveToWhitelist = async (id: number) => {
      const response: any = await removeToWhitelist(id);
   ...
   return (
      <BotProvider botId={Number(params.botId)}>
         <div className={styles.settings_container}>
            <div className={styles.header}>
               <div className={styles.header_title}>
                  Настрока бота {bot?.id}
               </div>
            </div>
                        ...
                        <div className={styles.block_content}>
                           <div className={styles.inner_block}>
                              <div className={styles.bot_rank}>
                                 Конец:{' '}
                                 {bot &&
                                    new Date(bot?.endDate).toLocaleString()}
                              </div>
                              <button
